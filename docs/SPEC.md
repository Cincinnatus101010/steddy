...
The product is **Steddy**. The npm package is **`steddy`**.

## Objective

Steddy is a stale-while-revalidate data-fetching library for React. It does the same job as `useSWR`, rebuilt with strict one-way layering so reliability bugs cannot leak across concerns.
...
- `error` is `unknown`. Fetchers throw whatever they throw; Steddy does not wrap it.
...
- One esbuild tree-shake test: `import { useSteddy }` must not contain plugin implementations.
...
- **Ask first:** GraphQL helpers, user-facing config knobs beyond `{ suspense: true }`, `{ keepPreviousData: true }`, `{ staleTime }`, `{ dedupTime }`, `{ refetchInterval }`, `{ refetchWhenHidden }`, `{ fallbackData }`, `{ onSuccess }`, `{ onError }`.
...
- `error` stays `unknown`. No `SteddyError` wrapper.
