...
The Steddy design doc left three questions open: cache GC, whether `mutate` exists outside React, and error typing. v1 needs a conservative default for each so layers stay simple and testable.
...
3. **`error` is `unknown`.** Wrapping every fetcher rejection in `SteddyError` would invent a shape fetchers did not throw and hide `instanceof` checks users already have. The store records whatever was thrown.
...
- Pros: no singleton
- Cons: cannot update cache from route loaders, websockets, or event handlers outside the component that called `useSteddy`
...
**Multiple hook subscribers:** `useSteddy` does not call `revalidate` when `isInFlight(key)` is already true, so two mounted components share one request instead of aborting each other.
